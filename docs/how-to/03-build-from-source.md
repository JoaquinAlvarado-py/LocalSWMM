...
The build compiles OpenSWMM with `emcmake cmake` and Ninja:

1. Locates `.tools/emsdk`, `.tools/vcpkg`, source `cmake/wasm`, build dir `build/wasm`; sources `emsdk_env.sh` if `emcmake` isn't on PATH.
...
4. `emcmake cmake -S cmake/wasm -B build/wasm -G Ninja` with flags:
...
5. `cmake --build build/wasm --parallel` (the static `libopenswmm.engine.a` links into the modular JS/WASM pair).
6. **Outputs** `public/swmm6wasm.wasm` and `public/swmm6wasm.js`.
7. **Stamps** `public/swmm6wasm.version.json` with `engineCommit`, `engineDescribe` (`git describe --always --dirty --tags`), `builtAtUtc`.

## 3. The vcpkg manifest & triplet

Root `vcpkg.json` deps: **`eigen3`, `hdf5`, `nlohmann-json`, `sundials`**.
...
- Compiles the engine C API and links against `openswmm_engine`.
- Outputs `swmm6wasm` into `public/`.
...
| `-s MODULARIZE=1`, `-s EXPORT_NAME=createOpenSwmm` | wrap output in the factory function |
