...
Every technology and framework in the project and its role — the UI, the map rendering, the engine, the server, and the tooling.
...
**CDN dependencies loaded by `index.html`** (all pinned, CSP-restricted to `api.mapbox.com`, `cdn.jsdelivr.net`, `cdnjs.cloudflare.com`, `unpkg.com`, `maps.googleapis.com`): `mapbox-gl-js@3.1.2`, `proj4js@2.9.0`, `shpjs@4.0.4`, `dxf-parser@1.1.2`, `geotiff@2.1.3`, and (optionally) the Google Maps JS API.
