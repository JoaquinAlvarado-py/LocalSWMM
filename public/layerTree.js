...
(function (window) {
    ...
    function refresh() {
        var body = document.getElementById('layer-tree-body'), m = window.map; if (!body) return;
        if (window.Mesh2DLayers) window.Mesh2DLayers.ensure(m);

        var s = state(); body.innerHTML = '';
        var group = '';
    ...
    window.LayerTree = { refresh: refresh, rows: rows, enableResultsDefaults: enableResultsDefaults };
